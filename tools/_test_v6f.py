# -*- coding: utf-8 -*-
"""v6f: v6e + 近白区域的描边环测试 —— 环带深色占比高=真部件(留), 低=烟团(杀)"""
import glob
import cv2
import numpy as np
...
def matte(arr, mn_floor):
    ...
    if keep.mean() > 0.45:
        ...
        if n3 > 1:
            ...
            keep = keepc
    # ---- v6f: 近白区域描边环测试 ----
    nw = ((mn >= 210) & (sat <= 55) & keep).astype(np.uint8)
    nw = cv2.morphologyEx(nw, cv2.MORPH_OPEN, K3)  # 去细噪声
    nr, labr, str_, _ = cv2.connectedComponentsWithStats(nw)
    removed = 0
    for i in range(1, nr):
        x, y, bw2, bh2, area = str_[i]
        if area < 2500:
            continue
        reg = (labr == i)
        ring = cv2.dilate(reg.astype(np.uint8), np.ones((9, 9), np.uint8)).astype(bool) & ~reg
        if ring.sum() < 50:
            continue
        dark_frac = float((mn[ring] < 170).mean())
        if dark_frac < 0.25:
            keep &= ~reg
            removed += area
    ...
    return keep.mean() * 100, resid, removed
...
for d, floor, note in CASES:
    ...
    k, resid, rm = matte(arr, floor)
    print('%-40s %-12s keep=%5.2f%% 残留=%5.2f%% 杀白区=%6dpx'
          % (d.split('/')[-2][:20] + '/' + d.split('/')[-1], note, k, resid, rm))
