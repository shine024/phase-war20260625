...
function render(eraFilter) {
  ...
  // Use mousedown on canvas, mousemove on document
...
function drawCanvas(idx, e) {
  ...
  // Large hit areas for dragging
  // Green point - foot (radius 20)
  ctx.fillStyle = 'rgba(80,200,120,0.4)';
  ctx.beginPath(); ctx.arc(centerX, footY, 20, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#50c878';
  ctx.beginPath(); ctx.arc(centerX, footY, 8, 0, Math.PI*2); ctx.fill();
  ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.stroke();
  
  // Green point - head (radius 20)
  ctx.fillStyle = 'rgba(80,200,120,0.4)';
  ctx.beginPath(); ctx.arc(centerX, headY, 20, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#50c878';
  ctx.beginPath(); ctx.arc(centerX, headY, 8, 0, Math.PI*2); ctx.fill();
  ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.stroke();
  
  // Red point - fire spawn (radius 25)
  ctx.fillStyle = 'rgba(255,64,64,0.35)';
  ctx.beginPath(); ctx.arc(centerX, fireY, 25, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#ff4040';
  ctx.beginPath(); ctx.arc(centerX, fireY, 10, 0, Math.PI*2); ctx.fill();
  ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.stroke();
...
function getPointInfo(idx, clickX, clickY) {
  ...
  const distToFoot = Math.abs(clickX - centerX) + Math.abs(clickY - footY);
  const distToHead = Math.abs(clickX - centerX) + Math.abs(clickY - headY);
  const distToFire = Math.abs(clickX - centerX) + Math.abs(clickY - fireY);
  
  // Use Manhattan distance for simpler calculation
  if (distToFire < 35) return { type: 'fire' };
  if (distToFoot < 28) return { type: 'ff' };
  if (distToHead < 28) return { type: 'hf' };
  return null;
}
