...
> ⚠️ **已废弃（v8.0，2026-08-17 确认）**：`data/base_unit_stats.gd` 已删除。
> 本文件的 L0 层设计被 **`data/unified_card_table.gd`（统一卡牌表，v8.0 单一真值源）** 取代——
> 玩家卡/敌方原型/缴获卡三者共享同一套基础数值，且数值量级与本表不同（本表数值已过时，
> 如 future_fighter 写 HP450，统一表实为 1175）。**调平衡请改 unified_card_table.gd**，
> 勿参考本文档数值。5 层加成模型（L1-L4）仍大体适用，仅 L0 数据源变更。
> 保留本文档仅作历史设计参考。
