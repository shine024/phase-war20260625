...
function render(eraFilter) {
  ...
  // Drag handlers - mousedown on canvas, mousemove/mouseup on document
...
function drawCanvas(idx, e) {
  ...
  // Draw interactive points with LARGE hit areas
  // Green point - foot (radius 25 for hit area)
  ctx.fillStyle = 'rgba(80,200,120,0.5)';
  ctx.beginPath(); ctx.arc(centerX, footY, 25, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#50c878';
  ctx.beginPath(); ctx.arc(centerX, footY, 10, 0, Math.PI*2); ctx.fill();
  ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.stroke();
  
  // Green point - head (radius 25 for hit area)
  ctx.fillStyle = 'rgba(80,200,120,0.5)';
  ctx.beginPath(); ctx.arc(centerX, headY, 25, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#50c878';
  ctx.beginPath(); ctx.arc(centerX, headY, 10, 0, Math.PI*2); ctx.fill();
  ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.stroke();
  
  // Red point - fire spawn (radius 30 for hit area)
  ctx.fillStyle = 'rgba(255,64,64,0.4)';
  ctx.beginPath(); ctx.arc(centerX, fireY, 30, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#ff4040';
  ctx.beginPath(); ctx.arc(centerX, fireY, 12, 0, Math.PI*2); ctx.fill();
  ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.stroke();
...
function getPointInfo(idx, clickX, clickY) {
  ...
  // Simple Y-distance check (points are vertically aligned)
  const distToFoot = Math.abs(clickY - footY);
  const distToHead = Math.abs(clickY - headY);
  const distToFire = Math.abs(clickY - fireY);
  
  // Check if click is near the center X as well
  const xNearCenter = Math.abs(clickX - centerX) < 40;
  
  if (xNearCenter && distToFire < 35) return { type: 'fire' };
  if (xNearCenter && distToFoot < 28) return { type: 'ff' };
  if (xNearCenter && distToHead < 28) return { type: 'hf' };
  return null;
}
