...
function render(eraFilter) {
  ...
  document.querySelectorAll('canvas').forEach(cv => {
    ...
    cv.addEventListener('mousedown', e => {
      ...
      if (point) {
        dragState = { idx, type: point.type, canvas: cv };
        e.preventDefault();
      }
    });
  });
  ...
  document.addEventListener('mousemove', e => {
    ...
    if (e.clientX >= rect.left && e.clientX <= rect.right &&
      ...
      const scaleY = cv.height / rect.height;
      const clickY = (e.clientY - rect.top) * scaleY;
      updatePoint(dragState.idx, dragState.type, clickY);
    }
  });
...
function drawCanvas(idx, e) {
  ...
  // Small hit areas
  // Green - foot (12px hit, 5px visible)
  ctx.fillStyle = 'rgba(80,200,120,0.6)';
  ctx.beginPath(); ctx.arc(centerX, footY, 12, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#50c878';
  ctx.beginPath(); ctx.arc(centerX, footY, 5, 0, Math.PI*2); ctx.fill();
  
  // Green - head (12px hit, 5px visible)
  ctx.fillStyle = 'rgba(80,200,120,0.6)';
  ctx.beginPath(); ctx.arc(centerX, headY, 12, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#50c878';
  ctx.beginPath(); ctx.arc(centerX, headY, 5, 0, Math.PI*2); ctx.fill();
  
  // Red - fire (14px hit, 6px visible)
  ctx.fillStyle = 'rgba(255,64,64,0.5)';
  ctx.beginPath(); ctx.arc(centerX, fireY, 14, 0, Math.PI*2); ctx.fill();
...
function getPointInfo(idx, clickX, clickY) {
  ...
  // Check X proximity (must be near center column)
  if (Math.abs(clickX - centerX) > 35) return null;
  ...
  // Return closest point if within threshold
  if (dFire < 20) return { type: 'fire' };
  if (dFoot < 16) return { type: 'ff' };
  if (dHead < 16) return { type: 'hf' };
  return null;
}

function updatePoint(idx, type, clickY) {
  const en = enemies[idx];
  const texH = 140, texW = 56;
  const texX = (140 - texW) / 2, texTop = 10;
  const texBot = texTop + texH;
