...
- 缴获卡面那 36 个（C 段战场敌人）在缴获卡篇注明为"任务出现单位"，并按 BOSS/精英/常规细分（见 §4）。
- 出现定位标签按 §4。
...
| `任务专属` | C 段 `enemy_`/`elite_` 前缀（缴获卡面常规+精英） | 31 |
| `BOSS` | C 段 `boss_*`（缴获卡面 5 个 BOSS） | 5 |
| `堡垒` | E 段 `fort_*` | 10 |
| **合计** | | **109** |

C 段 36 个明细：20 `enemy_` + 11 `elite_` + 5 `boss_`。

规则：`boss_*` 标 `BOSS`；`fort_*` 标 `堡垒`；C 段 `enemy_`/`elite_` 标 `任务专属`；A/B/D 段标 `主线波次`。

> 缴获卡面（vis_player_036~071）= C 段全部 36 个。文档缴获卡篇统一注明这 36 个为"任务出现单位"，并按 BOSS(5)/精英(11)/常规(20) 细分标签。
