...
.grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(400px,1fr)); gap:12px; padding:0 16px 40px; max-width:1600px; margin:0 auto; }
.card { background:#161b26; border:1px solid #2a2f3a; border-radius:8px; overflow:hidden; }
.card-header { display:flex; align-items:center; gap:10px; padding:8px 10px; border-bottom:1px solid #2a2f3a; }
.card-icon { width:56px; height:56px; object-fit:cover; border-radius:4px; background:#1e2430; border:1px solid #333; }
.card-info { flex:1; }
.card-name { font-size:13px; font-weight:600; }
.card-id { font-size:9px; color:#8892a4; font-family:monospace; margin-top:1px; }
.card-body { display:flex; gap:8px; padding:8px 10px; }
.canvas-wrap { position:relative; flex-shrink:0; }
.canvas-wrap canvas { display:block; border-radius:4px; cursor:pointer; background:#0d1117; }
.stats { flex:1; min-width:0; }
.stat-row { display:flex; justify-content:space-between; padding:2px 0; font-size:10px; border-bottom:1px solid rgba(255,255,255,0.04); }
.stat-label { color:#8892a4; }
.stat-value { font-weight:600; }
.wt-badge { display:inline-block; padding:1px 4px; border-radius:2px; font-size:9px; font-weight:700; color:#000; }
.edit-section { margin-top:4px; padding-top:4px; border-top:1px solid #2a2f3a; }
.edit-row { display:flex; align-items:center; gap:4px; padding:1px 0; }
.edit-row label { color:#8892a4; font-size:9px; min-width:42px; }
.edit-row input { width:48px; background:#0d1117; border:1px solid #2a2f3a; color:#e2e8f0; border-radius:2px; padding:1px 3px; font-size:9px; }
.edit-row input:focus { outline:none; border-color:#58a6ff; }
.card-footer { padding:4px 10px; border-top:1px solid #2a2f3a; font-size:9px; color:#8892a4; }
.formula { text-align:center; color:#8892a4; font-size:10px; padding:6px 16px; border-bottom:1px solid #2a2f3a; }
.formula code { background:#21262d; padding:1px 3px; border-radius:2px; color:#58a6ff; font-size:9px; }
.toast { position:fixed; top:16px; right:16px; padding:6px 12px; border-radius:4px; font-size:11px; z-index:999; opacity:0; transition:opacity .3s; pointer-events:none; }
...
<p class="formula">
  <span style="color:#50c878">■ 绿点</span> = 脚部/头部 (拖动调整实体) |
  <span style="color:#ff4040">● 红点</span> = 弹道起点 (拖动独立调整)<br>
...
function render(eraFilter) {
  ...
  list.forEach((e, li) => {
    ...
    card.innerHTML = `
      <div class="card-header" style="border-top:3px solid ${ERA_COLOR[e.era]}">
        ...
        <div class="card-info">
          <div class="card-name">${e.name}<span style="display:inline-block;font-size:8px;padding:1px 4px;border-radius:6px;background:${ERA_COLOR[e.era]}33;color:${ERA_COLOR[e.era]};margin-left:2px;">${e.era}</span></div>
          <div class="card-id">${e.id} → vis_enemy_${String(idx+36).padStart(3,'0')}</div>
        </div>
      </div>
      <div class="card-body">
        <div class="canvas-wrap">
          <canvas id="cv${idx}" width="140" height="180"></canvas>
        </div>
        <div class="stats">
      ...
      <div class="card-footer">
        <span style="color:#50c878">■ 绿</span>拖脚/头 &nbsp;|&nbsp; <span style="color:#ff4040">● 红</span>拖弹道
      </div>
    `;
  ...
  document.querySelectorAll('canvas').forEach(cv => {
    ...
    cv.addEventListener('mousedown', e => startDrag(idx, e, cv));
  });
...
function drawCanvas(idx, e) {
  ...
  ctx.setLineDash([]);
  ctx.fillStyle = '#50c878'; ctx.font = '8px sans-serif';
  ctx.fillText('脚 y='+footY.toFixed(0), texX+texW+4, footY+3);

  ctx.beginPath(); ctx.moveTo(texX-4, headY); ctx.lineTo(texX+texW+4, headY); ctx.stroke();
  ctx.fillText('头 y='+headY.toFixed(0), texX+texW+4, headY+3);

  ctx.fillStyle = '#ff6b6b'; ctx.font = 'bold 9px sans-serif';
  ctx.fillText('弹道起点', texX+texW+4, fireY-6);
  ctx.font = '8px monospace';
  ctx.fillText('顶部 '+firePct.toFixed(1)+'%', texX+texW+4, fireY+6);

  // Hit areas (larger than visible dots)
  // Green - foot (15px hit, 5px visible)
  ctx.fillStyle = 'rgba(80,200,120,0.5)';
  ctx.beginPath(); ctx.arc(centerX, footY, 15, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#50c878';
  ctx.beginPath(); ctx.arc(centerX, footY, 5, 0, Math.PI*2); ctx.fill();
  
  // Green - head (15px hit, 5px visible)
  ctx.fillStyle = 'rgba(80,200,120,0.5)';
  ctx.beginPath(); ctx.arc(centerX, headY, 15, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#50c878';
  ctx.beginPath(); ctx.arc(centerX, headY, 5, 0, Math.PI*2); ctx.fill();
  
  // Red - fire (18px hit, 6px visible)
  ctx.fillStyle = 'rgba(255,64,64,0.5)';
  ctx.beginPath(); ctx.arc(centerX, fireY, 18, 0, Math.PI*2); ctx.fill();
  ctx.fillStyle = '#ff4040';
  ctx.beginPath(); ctx.arc(centerX, fireY, 6, 0, Math.PI*2); ctx.fill();

  ctx.fillStyle = ERA_COLOR[e.era];
  ctx.fillRect(0, 0, 3, H);
}

function startDrag(idx, e, cv) {
  ...
  const en = enemies[idx];
  const texH = 140, texW = 56;
  const texX = (140 - texW) / 2, texTop = 10;
  const texBot = texTop + texH;
  ...
  // Check if click is near center column
  if (Math.abs(clickX - centerX) > 35) return;
  
  // Check vertical distances
  const dFoot = Math.abs(clickY - footY);
  const dHead = Math.abs(clickY - headY);
  const dFire = Math.abs(clickY - fireY);
  
  // Start drag if close enough
  if (dFire < 22) {
    dragState = { idx, type: 'fire' };
    e.preventDefault();
  } else if (dFoot < 18) {
    dragState = { idx, type: 'ff' };
    e.preventDefault();
  } else if (dHead < 18) {
    dragState = { idx, type: 'hf' };
    e.preventDefault();
  }
}
...
function onDragMove(e) {
  if (!dragState) return;
  const cv = document.getElementById('cv' + dragState.idx);
  if (!cv) return;
  ...
  const en = enemies[dragState.idx];
  const texH = 140, texW = 56;
  const texTop = 10;
  const texBot = texTop + texH;
  ...
  if (dragState.type === 'ff') {
    ...
    if (Math.abs(newFF - en.ff) > 0.001) {
      en.ff = newFF;
      en.fire_pct = calcFirePct(en);
      syncInputs(dragState.idx);
      drawCanvas(dragState.idx, en);
    }
...
html_path = r'F:/godot fair duet/create/phase-war/docs/enemy_fire_spawn.html'
