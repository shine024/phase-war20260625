...
Phase War（构装纪元）是一款 Godot 4.5 / GDScript 开发的自动战斗卡牌构筑游戏。玩家扮演相位指挥官，通过装配平台卡和武器卡构建作战单位，搭配相位法则和环境策略，在 100 个关卡中击败敌人。核心循环围绕"战前装配构筑，战中部署操作"展开，碎片永久积累驱动长期成长。游戏覆盖 5 个时代（一战→近未来），7 个势力，300+ 卡牌组合，12 种相位法则。
...
- **收集成长**：碎片永久积累，持续变强 — 蓝图 1-9 星渐进，法则碎片解锁新能力
...
- **战斗**: 完成敌人波次，保护相位场驱动器
- **结算**: 获得碎片、纳米材料、星级评价（1-3 星）

#### 会话循环（长期进程）
- **收集**: 战斗获得蓝图碎片、法则碎片（永久不消耗）
- **成长**: 蓝图星级提升（1-9★），解锁新法则
...
| 蓝图系统 | 碎片收集、星级、制造 | 碎片不消耗，9 星上限 |
| 合成系统 | 卡牌升级稀有度、蓝图融合 | 80%/90% 成功率 |
...
```
card_plus_card = 80% success (rare→rare, uncommon→uncommon)
card_plus_card = 90% success (common→uncommon)
result = randf() < success_rate
```

### 蓝图自动解锁阈值

```
unlock_threshold = BlueprintManager.get_fragment_count(card_id) >= required_fragments
```
...
- **合成失败**: 卡牌保留不消耗，纳米材料照扣
- **法则碎片类型**: 法则碎片使用 `law:` 前缀区分于普通蓝图碎片
- **9 星溢出转换**: 蓝图 9 星后多余碎片转换为纳米材料（50 纳米/碎片，每日上限 1000）
...
Feature (depends on core):
  ...
  Synthesis → Star Rating → Card Manufacturing

Presentation (depends on features):
  Combat UI → Blueprint Library UI → Law Library UI → Synthesis UI
...
| 蓝图溢出转换率 | 50 纳米/碎片 | blueprint-system | 9 星后碎片转化价值 |
| 蓝图溢出每日上限 | 1000 纳米 | blueprint-system | 每日碎片转化纳米上限 |
| 合成成功率 | 80%/90% | synthesis-system | 卡牌升级成功概率 |
| 合成纳米消耗 | 50/150/500 | synthesis-system | 各稀有度升级费用 |
