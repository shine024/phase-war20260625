...
## 隐藏分支：隐形轰炸机
## v7.x: 删除原制空线 mod_f16/fut_f22（死链），缩为 cold_mig21→fut_space_fighter
## v7.x: 删除原 STEALTH_BRANCH 的 fut_b2（死链），保留 fut_stealth_bomber 终端
...
const AIR_SUPERIORITY_LINE: Dictionary = {
	...
	E1 = {
		stage = 1, card_id = "fut_space_fighter", name = "空天战斗机",
...
const GROUND_ATTACK_LINE: Dictionary = {
	...
	E2 = {
		...
		era = "Future", power = 1300, max_hp = 380,
	...
	E3 = {
		...
		era = "Ultimate", power = 1325, max_hp = 300,
...
## 隐藏分支：隐形轰炸机（v7.x: 删除原 E2_STEALTH fut_b2 死链，保留 fut_stealth_bomber 终端）
const STEALTH_BRANCH: Dictionary = {
	E2_STEALTH = {
		stage = 2, card_id = "fut_stealth_bomber", name = "隐身轰炸机",
...
static func check_requirements(card: Dictionary, target_stage: String) -> Dictionary:
	push_warning("[AirEvolution] check_requirements 为废弃存根，请改用 BlueprintManager.can_evolve_blueprint / EvolutionPathRegistry.check_evolution_requirements")
