"""
Generate remaining missing player card icons using agnes-image model.

Only generates cards that DON'T already exist in assets/card_icons/.
Batch processing: 10 cards per batch with 5s delay between batches.

Usage:
  python scripts/generate_missing_card_icons_batch2.py
"""
...
# Cards that were NOT generated in batch 1 (WW1 cards already exist now)
CARDS = [
    # ═══════════ WW2 (18 cards) ═══════════
    ...
    ("mod_m6", "Modern M6 self-propelled AA, American radar-guided anti-aircraft gun on tracked chassis, 1960s era, realistic military illustration, isolated on white background, 1024x1024"),
    ...
    ("fut_howitzer", "Future self-propelled howitzer, anti-gravity artillery with electromagnetic rail gun, 2050s era, realistic military illustration, isolated on white background, 1024x1024"),
...
def main():
    ...
    for card_id, prompt in CARDS:
        if os.path.exists(os.path.join(OUTPUT_DIR, f"{card_id}.png")):
            skipped += 1
        else:
            to_generate.append((card_id, prompt))

    print(f"Total in list: {len(CARDS)}")
    ...
    for batch_start in range(0, total, BATCH_SIZE):
        ...
        batch = to_generate[batch_start:batch_end]

        print(f"--- Batch {batch_start // BATCH_SIZE + 1} ({batch_start + 1}-{batch_end}/{total}) ---")
        for i, (card_id, prompt) in enumerate(batch):
            idx = batch_start + i + 1
            print(f"[{idx}/{total}] Generating: {card_id}...")
            ...
            # Small delay between cards in same batch
            if i < len(batch) - 1:
                time.sleep(3)
    ...
    print(f"Note: {skipped} cards were already existing")
