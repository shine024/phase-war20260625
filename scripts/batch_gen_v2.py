# -*- coding: utf-8 -*-
"""
Phase War 无武器版资产批量生成脚本 v2
按优先级顺序生成，依次等待完成。

用法: python batch_gen_v2.py
"""
...
def gen_and_download(prompt, output_path):
    basename = os.path.basename(win_path(output_path))
    if os.path.exists(win_path(output_path)) and os.path.getsize(win_path(output_path)) > 5000:
        print(f"[SKIP] {basename} already exists", flush=True)
        return True
    print(f"[GEN] {basename} ...", flush=True)
    for attempt in range(5):
        ...
        if "上限" in msg:
            ...
            continue
        if "error" in result:
            print(f"[ERR] {json.dumps(result)}", flush=True)
            return False
        print(f"[WARN] Unexpected: {str(result)[:200]}", flush=True)
        time.sleep(5)
    return False

# ============================================================
# 任务清单: (文件名, 输出路径, prompt)
# ============================================================

import subprocess

TASKS = [
    # --- A. 卡牌边框 (B类，第2优先，但可与卡牌并行) ---
...
for i, (fname, out_path, prompt) in enumerate(TASKS, 1):
    print(f"\n[{i}/{len(TASKS)}] {fname}", flush=True)
    ok = gen_and_download(prompt, out_path)
    if ok:
        success += 1
    else:
