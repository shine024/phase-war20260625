...
## 执行攻击（指定伤害值）
static func do_attack_with_damage(u: CharacterBody2D, damage: float, weapon_type_override: int = -1) -> void:
	...
	if wt == 0 and u.stats and u.stats.attack_range > 0.5 and dist_t > u.stats.attack_range:
		...
		if att_mult <= 0.0:
			...
			damage *= att_mult
	elif dist_t > u.stats.attack_range and u.stats.attack_range > 0.5:
		var falloff: Dictionary = CombatTargeting.range_falloff(dist_t, u.stats.attack_range)
	...
	bullet.setup(u.target, damage, true, wt, u, u.stats, miss)
...
static func _process_single_weapon_attack(u: CharacterBody2D, delta: float) -> void:
	...
	var target_stats = u.target.get("stats") as UnitStats
	var target_kind: int = target_stats.combat_kind if target_stats else 0
	var timing: Dictionary = AttackCalculator.get_attack_timing(u.stats, target_kind)
	var fire_range: float = effective_fire_range(u)
	var dist: float = u.global_position.distance_to(u.target.global_position)
	if dist > fire_range and u.stats.weapon_type == GC.WeaponType.DIRECT:
		u._attack_phase = u.AttackPhase.IDLE
		u._attack_phase_timer = 0.0
		return
...
static func _process_multi_weapons(u: CharacterBody2D, delta: float) -> void:
	...
	for i in range(u._weapon_cfgs.size()):
		...
		if u.stats != null:
			var timing: Dictionary = AttackCalculator.get_attack_timing(u.stats, target_kind)
			if w_interval > 0 and u.stats.attack_interval > 0:
				var scale_ratio: float = w_interval / u.stats.attack_interval
		...
		match phase:
			...
			u.AttackPhase.ACTIVE:
				...
				if phase_timer < delta * 1.1:
					var dmg: float = float(w.get("damage", u.stats.attack_damage))
					do_attack_with_damage(u, dmg, w_wt)
