"""
Generate remaining missing player card icons - WITH RETRY

Generates cards that DON'T already exist.
Has retry logic for connection errors.

Usage:
  python scripts/generate_remaining_icons_retry.py
"""
...
# Remaining cards (starting from cold_btr60 where it failed)
CARDS = [
...
def generate_one_with_retry(card_id, prompt, max_retries=3):
    """Generate single image with retry on connection errors."""
...
def main():
    ...
    for i, (card_id, prompt) in enumerate(CARDS):
        ...
        ok, detail = generate_one_with_retry(card_id, prompt)
        if ok:
            success += 1
